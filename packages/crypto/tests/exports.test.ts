...
describe("@zig-wasm/crypto - Sync API", () => {
  ...
  // Note: This test would require dynamic module loading which doesn't work in Vitest
  // The NotInitializedError is properly thrown when using sync methods without init()
  // but testing it requires module isolation that conflicts with beforeAll hook
  it.skip("throws NotInitializedError when not initialized", () => {
    // Would need to test in separate process or with module mocking
  });
});
