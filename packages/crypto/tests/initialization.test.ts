...
describe("@zig-wasm/crypto - Init with wasmUrl option", () => {
  // Note: wasmUrl with file:// protocol doesn't work due to fetch limitations
  // This branch is primarily for browser environments with http/https URLs
  it.skip("initializes with wasmUrl (browser only)", async () => {
    vi.resetModules();
    const freshCrypto = await import("../src/crypto.ts");
    // Would need http:// URL for this to work - browsers can fetch from URLs
    const wasmUrl = "http://localhost/crypto.wasm";
    await freshCrypto.init({ wasmUrl });
    expect(freshCrypto.isInitialized()).toBe(true);
  });
});
